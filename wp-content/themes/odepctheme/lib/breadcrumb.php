...
function bootstrap_breadcrumb() {
  ...
  if ( !is_front_page() ) {
      ...
      if ( is_archive() && !is_tax() && !is_category() && !is_tag() ) {
          echo '<li class="breadcrumb-item active item-archive">' . post_type_archive_title($prefix, false) . '</li>';
          ...
          echo '<li class="breadcrumb-item active item-archive">' . $custom_tax_name . '</li>';
      } else if ( is_single() ) {
          // If post is a custom post type
          $post_type = get_post_type();
          // If it is a custom post type display name and link
          ...
          // Category page
          echo '<li class="breadcrumb-item active item-cat">' . single_cat_title('', false) . '</li>';
      } else if ( is_page() ) {
          // Standard page
          if( $post->post_parent ){
          ...
          echo '<li class="breadcrumb-item active active-' . get_the_time('Y') . '">' . get_the_time('Y') . ' Archives</li>';
      } else if ( is_author() ) {
          // Auhor archive
          ...
          // Display author name
          echo '<li class="breadcrumb-item active active-' . $userdata->user_nicename . '">' . 'Author: ' . $userdata->display_name . '</li>';
      } else if ( get_query_var('paged') ) {
          // Paginated archives
          echo '<li class="breadcrumb-item active active-' . get_query_var('paged') . '">'.__('Page') . ' ' . get_query_var('paged') . '</li>';
      } else if ( is_search() ) {
          // Search results page
          echo '<li class="breadcrumb-item active active-' . get_search_query() . '">Search results for: ' . get_search_query() . '</li>';
      } elseif ( is_404() ) {
          // 404 page
          echo '<li class="breadcrumb-item active Error-404">' . 'Error 404' . '</li>';
      }
      echo '</ol>';
  }
}
// search result breadcrumb
function search_breadcrumb(){
$separator          = ' / ';
$breadcrums_id      = 'breadcrumbs';
$breadcrums_class   = 'breadcrumb';
$home_title         = 'Home';
